...
**Weapons are excluded from the armor stat budget entirely.** Weapon identity expresses through its own perks, its weapon class, and Steadfast behavior — not by rolling Resolve. The ten armor slots are the whole stat surface. *(The weapon side of power and variance is now specified elsewhere: damage scales by the weapon's own tier — WEAPON_ARCHETYPES.md, DECIDED 2026-08-04 — and per-drop rolled perks are the weapon variance axis per WEAPON_PERK_TABLES.md: 0 rolls through Remembered, Resonant 3 (one per bucket), Pure trait + 3. Weapons also cost Strain at Resonant/Pure from the same 100-point pool as armor. Draft for review.)*
